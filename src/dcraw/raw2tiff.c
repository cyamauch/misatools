			...
			const long crop_prms[] )
{
    ...
    TIFF *tiff_out = NULL;
    uint32 shift_for_scale_r = 16 - bit_used[0];
    uint32 shift_for_scale_g = 16 - bit_used[1];
    uint32 shift_for_scale_b = 16 - bit_used[2];
    ...
    if ( deadpix_file != NULL && 0 < strlen(deadpix_file) ) {
        ...
        snprintf(dcraw_cmd, dcraw_cmd_len,
		 "dcraw -c -D -4 -j -t 0 -P %s %s", deadpix_file, filename_in);
    }
    else {
	snprintf(dcraw_cmd, dcraw_cmd_len,
		 "dcraw -c -D -4 -j -t 0 %s", filename_in);
    }
...
int main( int argc, char *argv[] )
{
	...
	fprintf(stderr,"\n");
        ...
        fprintf(stderr,"$ %s [-P file] [-c param] bit_used rawfile1 rawfile2 ...\n",argv[0]);
        fprintf(stderr,"$ %s [-P file] [-c param] bit_used_R,bit_used_G,bit_used_B rawfile1 ...\n",argv[0]);
	fprintf(stderr,"\n");
    ...
    while ( arg_cnt < argc ) {
        if ( raw_to_tiff(argv[arg_cnt],
			 deadpix_file, bit_used, crop_prms) < 0 ) {
