...
static int read_tiff24or48( const char *filename_in,
			    mdarray *ret_img_buf, mdarray_uchar *ret_icc_buf )
{
    stdstreamio sio;
	...
	if ( ret_img_buf != NULL ) {
	    ...
	    ret_r_img_ptr = (unsigned char *)ret_img_buf->data_ptr(0,0,0);
	    ret_g_img_ptr = (unsigned char *)ret_img_buf->data_ptr(0,0,1);
	    ret_b_img_ptr = (unsigned char *)ret_img_buf->data_ptr(0,0,2);
	}
	...
	if ( ret_img_buf != NULL ) {
	    ...
	    ret_img_buf->resize_3d(width,height,3);
	    /* get array ptr of each ch */
	    ret_r_img_ptr = (float *)ret_img_buf->data_ptr(0,0,0);
	    ret_g_img_ptr = (float *)ret_img_buf->data_ptr(0,0,1);
	    ret_b_img_ptr = (float *)ret_img_buf->data_ptr(0,0,2);
	}
